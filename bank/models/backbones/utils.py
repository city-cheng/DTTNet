...
def set_requires_grad(model: nn.Module, keywords: List[str], all=False):
    """
    notice:key in name!
    """
    requires_grad_names = []
...
def _set_train(model: nn.Module, keywords: List[str], prefix: str = "", all=False):
    train_names = []
...
def set_train(model: nn.Module, keywords: List[str], all=False):
    """
    notice:sub name startwith key!
    """
    model.train(False)
    train_names = _set_train(model, keywords, all)

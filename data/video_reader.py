...
from utils.transforms import im_normalization
...
class VideoReader(Dataset):
    ...
    def __init__(
        ...
        # size = Image.open(path.join(mask_dir, sorted(os.listdir(mask_dir))[0])).size
        ...
        if size < 0:
            ...
            self.im_transform = transforms.Compose(
                [
                    transforms.ToTensor(),
                    transforms.Resize(
                        (size,size), interpolation=InterpolationMode.BILINEAR, antialias=False
                    ),
                    im_normalization,
                ]
            )
    ...
    def __getitem__(self, idx):
        ...
        #     mask = Image.open(gt_path).convert("1")
        #     mask = self.gt_transform(mask)
        #     data["mask"] = mask

        info["shape"] = shape
        info["need_resize"] = not (
            (type(self.size) != tuple and self.size < 0)
            or (type(self.size) == tuple and self.size[0] < 0)
        )
        data["image"] = img
        data["info"] = info
    ...
    def resize_mask(self, mask):
        ...
        h, w = mask.shape[-2:]
        min_hw = min(h, w)
        return F.interpolate(mask, (self.size[0], self.size[1]), mode="nearest")

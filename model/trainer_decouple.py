...
from utils.losses import SASLossComputer_decouple
...
class SASTrainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        self.loss_computer = SASLossComputer_decouple(config)
    ...
    def do_pass(self, data, it=0):
        ...
        body_gt = data["body"]  # b,t,1,h,w
        detail_gt = data["detail"]  # b,t,1,h,w
        ...
        with torch.cuda.amp.autocast(enabled=self.config["amp"]):
            ...
            logits, bodys, details = self.model(images, gt, descriptions)

            logits = torch.nn.functional.interpolate(
                logits, size=self.config["scale"], mode="bilinear", align_corners=False
            )  # b*t,1,h,w
            # for i in range(len(bounds)):
            bodys = [torch.nn.functional.interpolate(
                body, size=self.config["scale"], mode="bilinear", align_corners=False
            ) for body in bodys]  # b*t,1,h,w
            details = torch.nn.functional.interpolate(
                details, size=self.config["scale"], mode="bilinear", align_corners=False
            )  # b*t,1,h,w
            gt = gt.flatten(0, 1)
            images = images.flatten(0, 1)
            body_gt = body_gt.flatten(0, 1)
            detail_gt = detail_gt.flatten(0, 1)

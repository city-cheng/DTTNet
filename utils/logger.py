...
class TensorboardLogger:
    ...
    def log_cv2(self, tag, x, step,size=(384,384),nrow=2):
        """
            要求图片shape: B,C,H,W
        """
        if self.no_log:
            ...
            return
        # x = x.transpose((2, 0, 1))
        x = transforms.Resize(size,antialias=False)(x)
        grid = make_grid(x,nrow)
        # print(f'x shape:{grid.shape},x type:{grid.dtype}')
        self.logger.add_image(tag, grid, step)
